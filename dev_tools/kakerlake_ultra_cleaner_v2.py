...
# -*- coding: utf-8 -*-
"""
kakerlake_ultra_cleaner_v2.py
- Verbessertes Logging (PARSE_ERROR im Report)
- Robustere Erkennung für annual_savings (auch verschachtelte Dicts wie analysis_results, results_for_display, economics_data)
- Debug-Ausgabe (--debug) für Autopatcher
"""
from __future__ import annotations
import os, re, sys, ast, csv, argparse, hashlib
    ...
    def process(path:str)->bool:
        ...
        class Tx(cst.CSTTransformer):
            ...
            def leave_Assign(self, orig, updated):
                # Ziel muss Name annual_savings sein
                try:
                ...
                if m.matches(rhs, m.Call(func=m.Attribute(attr=m.Name("get")))):
                    call = rhs
                    first = call.args[0].value if call.args else None
                    if first and m.matches(first, m.SimpleString()):
                        ...
                        if key in RESULT_KEYS:
                            ...
                            self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "results_get_to_compute")
                    ...
                    try:
                        if m.matches(sub.slice, m.Index(value=m.SimpleString())):
                            ...
                            if key in RESULT_KEYS:
                                ...
                                self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "dict_index_to_compute")
                                return updated.with_changes(value=new_call)
                    except Exception:
                        pass
                ...
                if m.matches(rhs, m.BinaryOperation(operator=m.Add())):
                    ...
                    def name_in(node, names:Set[str]):
                        if m.matches(node, m.Name()): return node.value if node.value in names else None
                    ...
                    if f1 and c1:
                        ...
                        self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "feedin_plus_consumption")
                        return updated.with_changes(value=new_call)
                # 4) old - hp
                if m.matches(rhs, m.BinaryOperation(operator=m.Subtract())):
                    ...
                    if name_is(l, OLD_COST_NAMES) and name_is(r, HP_COST_NAMES):
                        ...
                        self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "old_minus_hp")
                        return updated.with_changes(value=new_call)
                ...
                diff = diff_names(rhs)
                if diff:
                    new_call = cst.parse_expression(f"compute_annual_savings(electricity_costs_without_pv={diff[0].code}, electricity_costs_with_pv={diff[1].code}, default=0.0)")
                    self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "without_with_difference")
                    return updated.with_changes(value=new_call)
                if m.matches(rhs, m.BinaryOperation(operator=m.Add())):
                    l=rhs.left; r=rhs.right
                    d=diff_names(l)
                    if d and r.code and any(k in r.code for k in FEEDIN_EXTRA):
                        new_call = cst.parse_expression(f"compute_annual_savings(electricity_costs_without_pv={d[0].code}, electricity_costs_with_pv={d[1].code}, annual_feed_in_revenue={r.code}, default=0.0)")
                        self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "diff_plus_feedin")
                        return updated.with_changes(value=new_call)
                    d=diff_names(r)
                    if d and l.code and any(k in l.code for k in FEEDIN_EXTRA):
                        new_call = cst.parse_expression(f"compute_annual_savings(electricity_costs_without_pv={d[0].code}, electricity_costs_with_pv={d[1].code}, annual_feed_in_revenue={l.code}, default=0.0)")
                        self.changed=True; log(path, getattr(orig,'ln','?'), orig_code, new_call.code, "feedin_plus_diff")
                        return updated.with_changes(value=new_call)
                return updated
...
# ---------- Autopatch project_data (wie gehabt) ----------
            ...
            def leave_SimpleStatementLine(self, orig, updated):
                if len(orig.body)==1 and m.matches(orig.body[0], m.Expr(value=m.Call(func=m.Attribute()))):
                    ...
                    if m.matches(call.func, m.Attribute(value=m.Name("project_data"), attr=m.Name("update"))):
                        ...
                        self.changed=True; log(path, getattr(orig,'ln','?'), call.code, repl.code.strip(), "project_data.update")
                        return repl
                return updated
            def leave_Assign(self, orig, updated):
                ...
                if m.matches(rhs, m.Dict()):
                    ...
                    log(path, getattr(orig,'ln','?'), rhs.code, new.code, "dict_literal_to_builder")
                ...
                if m.matches(rhs, m.BinaryOperation(operator=m.BitOr())):
                    ...
                    log(path, getattr(orig,'ln','?'), rhs.code, new.code, "dict_union_to_builder")
                    return updated.with_changes(value=new)
                if m.matches(rhs, m.Call()):
                    ...
                    log(path, getattr(orig,'ln','?'), rhs.code, new.code, reason)
                    return updated.with_changes(value=new)
                return updated
...
def do_structure(root:str, out:str):
    ...
    # Trees omitted here for brevity
...
def yaml_positions_check(root:str, out:str, tol_mm:float=0.2):
    ...
    # omitted: same as previous version

# ---------- CLI ----------
def main():
    ap=argparse.ArgumentParser(description="Kakerlake Ultra Cleaner v2 (debuggable)")
