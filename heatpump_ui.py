        ...
        if demand_mode == 'wp_only':
            ...
            if isinstance(project_data_effective, dict) and project_data_effective:
                ...
                if demand_mode == 'pv_wp_combined':
                    st.warning("⚡ **PV + Wärmepumpe-Modus:** Bitte führen Sie zuerst die PV-Analyse durch.")
                else:
                    st.info("PV-Daten optional. Für PV+WP-Integration bitte zuerst PV-Analyse durchführen.")
                pv_integration_data = None
        ...
        if st.button(" Ergebnisse als PDF exportieren"):
            try:
                ...
                analysis_results_hp = {
                    ...
                    'total_investment_netto': (st.session_state.get('heatpump_offer', {}) or {}).get('beg_subsidy', {}).get('effective_total_after_subsidy_net')
                }
                company_info = st.session_state.get('active_company_info', {}) or {}
                pdf_bytes = generate_offer_pdf(
                    project_data=project_data_hp,
                    analysis_results=analysis_results_hp,
                    company_info=company_info,
                    company_logo_base64=company_info.get('logo_base64'),
                    selected_title_image_b64=None,
                    selected_offer_title_text='Angebot Wärmepumpe',
                    selected_cover_letter_text='Sehr geehrte Damen und Herren,\n\nwir freuen uns, Ihnen unser Wärmepumpen-Angebot zu präsentieren.',
                    sections_to_include=None,
                    inclusion_options={'include_company_logo': True, 'include_product_images': False},
                    load_admin_setting_func=lambda k,d=None: d,
                    save_admin_setting_func=lambda k,v: False,
                    list_products_func=lambda *a, **k: [],
                ...
                if pdf_bytes:
                    st.download_button(" Wärmepumpen-Angebot PDF herunterladen", data=pdf_bytes, file_name="Waermepumpe_Angebot.pdf", mime="application/pdf")
                else:
