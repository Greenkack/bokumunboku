...
def _render_overview_section(results: Dict[str, Any], texts: Dict[str, str], viz_settings: Dict[str, Any]):
    ...
    • Jährliche Stromkosten (ohne Photovoltaik): {annual_total_costs_without_pv:.2f}€
    ...
    with fcol1:
        # Kosten OHNE Photovoltaik-Anlage (Referenz)
        ...
        # Kosten MIT Photovoltaik-Anlage (tatsächliche Kosten)
        ...
        st.metric(
            " Stromkosten OHNE Photovoltaik (10 Jahre)",
            format_kpi_value(costs_without_pv_10_with_increase, "€", texts_dict=texts, precision=0),
            delta=f"MIT Photovoltaik: {format_kpi_value(costs_with_pv_10_with_increase, '€', texts_dict=texts, precision=0)}"
        )
    ...
    with fcol2:
        ...
        st.metric(
            " Stromkosten OHNE Photovoltaik (20 Jahre)",
            format_kpi_value(costs_without_pv_20_with_increase, "€", texts_dict=texts, precision=0),
            delta=f"MIT Photovoltaik: {format_kpi_value(costs_with_pv_20_with_increase, '€', texts_dict=texts, precision=0)}"
        )
    ...
    with fcol3:
        ...
        st.metric(
            " Stromkosten OHNE Photovoltaik (30 Jahre)",
            format_kpi_value(costs_without_pv_30_with_increase, "€", texts_dict=texts, precision=0),
            delta=f"MIT Photovoltaik: {format_kpi_value(costs_with_pv_30_with_increase, '€', texts_dict=texts, precision=0)}"
        )
        ...
        chart_type = st.selectbox(
            ...
            index=(
                ...
                if default_chart_type == "Säulen"
                    ...
                    ["Säulen", "Balken", "Strich", "Kreis", "Donut"].index(default_chart_type)
                    if default_chart_type in ["Säulen", "Balken", "Strich", "Kreis", "Donut"]
                    else 0
                )
            ),
        ...
        if isinstance(data, dict) and "categories" in data and "series" in data:
            ...
            if chart_type == "Säulen":
                ...
                for col in df.columns[1:]:
                    agg_values[col] = df[col].sum()
                pie_df = pd.DataFrame({"Reihe": list(agg_values.keys()), "Wert": list(agg_values.values())})
                hole = 0.4 if chart_type == "Donut" else 0
                fig = px.pie(
                    pie_df,
                    names="Reihe",
...
def render_daily_production_switcher(
    ...
    df_day = pd.DataFrame({
        "Stunde": hours,
        get_text(texts, "pv_power_label_switcher", "Photovoltaik-Leistung (kW)"): power,
    })
    ...
    pv_label = get_text(texts, "pv_power_label_switcher", "Photovoltaik-Leistung (kW)")
    fig = go.Figure()
    if chart_type in ("Säulen", "Balken"):
        if chart_type == "Säulen":
            fig.add_trace(go.Bar(x=hours, y=power, name=pv_label))
        else:
            fig.add_trace(go.Bar(y=hours, x=power, orientation="h", name=pv_label))
    elif chart_type == "Strich":
        fig.add_trace(
            go.Scatter(x=hours, y=power, mode="lines+markers", name=pv_label)
        )
    elif chart_type in ("Kreis", "Donut"):
    ...
    if chart_type not in ("Kreis", "Donut"):
        fig.update_layout(
            title=title,
...
def render_production_vs_consumption_switcher(
    ...
    with st.expander("Verbrauch vs. Produktion (Jahr 1)", expanded=False):
        col1, col2, col3 = st.columns([2, 1, 1])
    ...
    if chart_type == "Säulen":
        ...
        if chart_type == "Säulen":
            ...
            fig = px.bar(
                df_comparison,
                y="Monat",
                x=["Verbrauch (kWh)", "Produktion (kWh)"],
                barmode="group",
                orientation="h",
                title="Verbrauch vs. Produktion (Jahr 1)",
                color_discrete_sequence=colors,
...
def render_tariff_cube_switcher(
    ...
    alternative Szenarien. Ergebnisse werden in analysis_results als Bytes
    für PDF-Export abgelegt.
    """
    title = get_text(
    ...
    # Daten vorbereiten
    anbieter = ["Aktuell", "Photovoltaik-Strom", "Alternativ A", "Alternativ B"]
    ...
    chart_data = {
        ...
        "series": [
            {
                "name": get_text(texts, "annual_costs_label_switcher", "Jahreskosten (€)"),
                "data": gesamt,
            }
        ],
    }
...
def render_co2_savings_value_switcher(
    ...
    if fig:
        with st.expander(co2_title, expanded=False):
            st.plotly_chart(
                fig,
                use_container_width=True,
...
def render_extended_calculations_dashboard(
    ...
    with tabs[3]:  # Degradation
        ...
        with col1:
            # Guard gegen Division durch 0 (system_kwp kann bei fehlerhaften / unvollständigen Eingaben 0 sein)
            _system_kwp = system_data.get("system_kwp") or 0
            delta_10 = (
                f"{((deg_results['power_year_10_kwp'] / _system_kwp) - 1) * 100:.2f}%"
                if _system_kwp > 0
                else "n/a"
            )
            st.metric(
        ...
        with col2:
            delta_20 = (
                f"{((deg_results['power_year_20_kwp'] / _system_kwp) - 1) * 100:.2f}%"
                if _system_kwp > 0
                else "n/a"
            )
            st.metric(
...
def render_investment_value_switcher(
    ...
    massnahmen_labels = [
        ...
        get_text(texts, "measure_pv_heatpump_switcher", "Photovoltaik + Wärmepumpe"),
    ]
...
def render_scenario_comparison_switcher(
    ...
    with st.expander(scenario_comp_subheader, expanded=False):
        st.plotly_chart(
            fig,
            use_container_width=True,
...
def render_tariff_comparison_switcher(
    ...
    kosten_vorher = [m_cons * elec_price for m_cons in m_total_cons]  # Kosten ohne Photovoltaik
    kosten_nachher = [
        m_grid * elec_price for m_grid in m_grid_draw
    ]  # Kosten mit Photovoltaik (nur Netzbezug)

    #  MODERNE 2D STROMKOSTEN VORHER/NACHHER 
    chart_data = {
        "categories": month_labels,
        "series": [
            {"name": "Kosten Vorher (ohne Photovoltaik)", "data": kosten_vorher},
            {"name": "Kosten Nachher (mit Photovoltaik)", "data": kosten_nachher},
        ],
    }
    ...
    df_monthly = pd.DataFrame(
        {
            ...
            get_text(
                texts_local, "pv_production_chart_label", "Photovoltaik Produktion (kWh)"
            ): monthly_prod,
            get_text(
    ...
    if selected_chart_type == "line":
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines+markers",
                name=get_text(
                    texts_local, "pv_production_chart_label", "Photovoltaik Produktion"
                ),
            )
        )
        fig.add_trace(
        ...
        fig.add_trace(
            go.Scatter(
                ...
                fill="tozeroy",
                name=get_text(
                    texts_local, "photovoltaik_production_chart_label", "Photovoltaik Produktion"
                ),
            )
        )
        fig.add_trace(
...
def _render_consumption_coverage_pie(
    ...
    if total_cons > 0:
        ...
        values = [self_supply_float, grid_cons_float]
        # Filter raus sehr kleine Anteile
        filtered = [(l, v) for l, v in zip(labels, values) if v >= 0.01]
        if not filtered:
            st.info(
                get_text(
                    texts_local,
...
def _render_pv_usage_pie(
    ...
    # Vier-Typ-Auswahl statt nur Pie
    ...
    if annual_pv_prod_kwh_val > 0:
        ...
        values = [direct_cons_float, storage_cons_float, feed_in_prod_perc]
        filtered = [(l, v) for l, v in zip(labels, values) if v >= 0.01]
        if not filtered:
            st.info(
                get_text(
        ...
        chart_data = {"labels": list(labels), "values": list(values)}
        title = get_text(
            texts_local, "pie_chart_pv_usage_title", "Aufteilung Photovoltaik-Produktion"
        )
        fig = create_four_type_chart(
            chart_data,
            title=title,
...
def render_environmental_calculations(
    ...
    with st.expander("Kreislaufwirtschaft & Recycling", expanded=False):
        ...
        fig = px.pie(
            ...
            values="weight_kg",
            names="material",
            title="Materialzusammensetzung der Photovoltaik-Anlage",
            color_discrete_map={
                "Silizium": "#3B82F6",
                "Aluminium": "#6B7280",
                "Glas": "#10B981",
...
def render_analysis(
    ...
    # Aktueller Angebotsmodus aus Session State lesen (wird in data_input.py gesetzt)
    demand_mode = st.session_state.get('demand_mode_selection', None)
    project_inputs = st.session_state.get("project_data", {})
    
    # Kurzer Status-Hinweis für den Nutzer mit benutzerfreundlichen Namen
    mode_names = {
        'Nur Photovoltaik': 'Photovoltaik',
        'Nur Wärmepumpe': 'Wärmepumpe',
        'Photovoltaik + Wärmepumpe': 'Photovoltaik & Wärmepumpe'
    }
    mode_display = mode_names.get(demand_mode, demand_mode or 'Nicht ausgewählt')
    st.info(f"**Aktiver Angebotsmodus:** {mode_display} | Änderung über 'Projekt - Bedarfsanalyse'")
    ...
    try:
        ...
        # NEU: Falls Wärmepumpen-Modus aktiv: relevante Bedarfsanalyse-Daten in Session heatpump_offer spiegeln
        if demand_mode in ('wp_only', 'pv_wp_combined'):
            try:
                dm = st.session_state.get('demand_mode_selection')
                if dm in ('wp_only', 'pv_wp_combined') and 'heatpump_offer' in st.session_state:
                    # Heizungskosten und Verbrauchsdaten aus project_data extrahieren und in HP-Angebot integrieren
                    hp_offer = st.session_state['heatpump_offer']
                    if not hp_offer.get('demand_inputs'):
                        hp_offer['demand_inputs'] = {}
                    # Mapping der Bedarfsanalyse-Daten
                    heating_month = project_inputs.get('stromkosten_heizung_euro_monat') or project_inputs.get('project_details', {}).get('stromkosten_heizung_euro_monat')
                    heating_year = project_inputs.get('stromkosten_heizung_euro_jahr') or project_inputs.get('project_details', {}).get('stromkosten_heizung_euro_jahr')
                    heating_type = project_inputs.get('heizungsart') or project_inputs.get('project_details', {}).get('heizungsart')
                    if heating_month:
                        hp_offer['demand_inputs']['heating_costs_monthly'] = float(heating_month)
                    if heating_year:
                        hp_offer['demand_inputs']['heating_costs_yearly'] = float(heating_year)
                    if heating_type:
                        hp_offer['demand_inputs']['heating_type'] = str(heating_type)
                    st.session_state['heatpump_offer'] = hp_offer
            except Exception as e_hp_data:
                st.warning(f"Warnung beim Übertragen der Heizungsdaten: {e_hp_data}")
    ...
    if MODERN_CHARTS_AVAILABLE:
        ...
        if fig_monthly_comp:
            ...
            with st.expander("Monatlicher Vergleich: Produktion vs. Verbrauch (Jahr 1)", expanded=False):
                st.plotly_chart(
                    fig_monthly_comp,
                    use_container_width=True,
        ...
        if fig_cost_projection:
            ...
            with st.expander("Stromkosten-Hochrechnung ohne Photovoltaik", expanded=False):
                st.plotly_chart(
                    fig_cost_projection,
                    use_container_width=True,
        ...
        with col_chart2:
            st.markdown("##### Photovoltaik-Strom Nutzung")
            # Pie Chart modernisieren
            if CHART_MODERNIZER_AVAILABLE:
                from universal_chart_modernizer import create_unified_pie_chart
        ...
        if fig_cum_cf:
            ...
            with st.expander("Kumulierter Cashflow über die Laufzeit (2D)", expanded=False):
                st.plotly_chart(
                    fig_cum_cf,
                    use_container_width=True,
...
def render_financing_analysis(
    ...
    with st.expander(" ROI-Analyse mit Finanzierungsoptionen", expanded=False):
        ...
        # Photovoltaik-Erträge aus bestehenden Berechnungen
    ...
    if annual_pv_revenue > 0:
        ...
        with col_tax3:
            st.metric("Jährlicher Photovoltaik-Ertrag", f"{annual_Photovoltaik_revenue:,.2f} €")
        with col_tax4:
...
def render_advanced_financial_analysis(results: Dict[str, Any], texts: Dict[str, str]):
    ...
    if investment > 0 and annual_savings > 0:
        ...
        with st.expander('Cashflow-Entwicklung über 20 Jahre', expanded=False):
...
def render_advanced_environmental_analysis(results: Dict[str, Any], texts: Dict[str, str]):
    ...
    # Fallback: Schätze CO2-Einsparung aus Photovoltaik-Produktion
    if co2_annual <= 0:
