...
class PDFGenerator:
    ...
    def create_pdf(self):
        ...
        for i, module_spec in enumerate(self.module_order):
            ...
            if draw_function:
                if progress_bar:
                    progress_bar.update(30 + (i * 40 // len(self.module_order)), f"Erstelle Modul: {module_id}")
...
def generate_main_template_pdf_bytes(
    ...
    wp_coords_available = coords_dir_wp.exists() and any(coords_dir_wp.glob('wp_seite*.yml'))
    if debug_templates:
    ...
    try:
        ...
        # Seitenzahl-Information in dynamische Daten injizieren für spezifische Placeholders
        ...
        if 'Photovoltaik' in segment_order:
            overlay_bytes_pv = generate_overlay(coords_dir_pv, dyn_data, total_pages=total_pages)
            overlay_parts.append(overlay_bytes_pv)
        if 'Wärmepumpe' in segment_order and wp_coords_available:
            # Für Wärmepumpe separate dyn_data (eigene Firmeninfo? project_data.company_information_wp)
            wp_company = project_data.get('company_information_wp') or company_info
            dyn_data_wp = build_dynamic_data(project_data, analysis_results, wp_company)
            overlay_bytes_wp = generate_overlay(coords_dir_wp, dyn_data_wp, total_pages=total_pages)
            overlay_parts.append(overlay_bytes_wp)
        # Merge Overlay Streams sequenziell (einfaches Aneinanderfügen der Seiten)
        ...
        print(f"[TEMPLATE] Fehler bei Overlay/Merge der 7-Seiten-PDF: {e_gen}")
        try:
            traceback.print_exc()
        except Exception:
...
def generate_offer_pdf_with_main_templates(
    ...
    **kwargs,
    ...
    1) Erzeuge 7-seitige Hauptausgabe per Templates (coords + notext PDFs)
    2) Erzeuge bisheriges PDF als Zusatz (ohne Deckblatt/Anschreiben)
    3) Hänge es an die 7 Seiten an und liefere Bytes
    """
    # Erzeuge optional die Zusatz-PDF zuerst, damit die 7-Seiten-Nummerierung "von XX" korrekt ist
    ...
    additional_pdf = None
    try:
        append_after_main7 = bool((inclusion_options or {}).get("append_additional_pages_after_main7", False))
    except Exception:
        append_after_main7 = False

    # Defensiv: Kopien anlegen und fehlende Felder (company_information, final_price) ergänzen,
    # damit build_dynamic_data in der 7-Seiten-Engine keine Warnungen "final_price" / Firmendaten auslöst.
    safe_project_data = dict(project_data or {})
    safe_analysis_results = dict(analysis_results or {}) if analysis_results else {}
    try:
        ...
        if fp_val in (None, 0, 0.0):
            try:
                ...
                if isinstance(live.get('final_price'), (int, float)) and live.get('final_price') > 0:
        ...
        if fp_val in (None, 0, 0.0):
            ...
            fp_val = next((v for v in candidates if isinstance(v, (int, float)) and v > 0), None)
        if isinstance(fp_val, (int, float)) and fp_val > 0:
            safe_analysis_results['final_price'] = fp_val
    except Exception:
        pass

    if append_after_main7:
        ...
        try:
            if additional_pdf and _PYPDF_AVAILABLE:
                ...
                print(f"[PDF EXTENDED] Zusatz-PDF erzeugt: {_dbg_pages} Seiten (append_after_main7=True)")
            else:
                print("[PDF EXTENDED] Zusatz-PDF leer oder pypdf nicht verfügbar – keine zusätzlichen Seiten angehängt")
        except Exception as _dbg_e:
            print(f"[PDF EXTENDED] Debug-Auswertung Zusatz-PDF fehlgeschlagen: {_dbg_e}")

    main7 = generate_main_template_pdf_bytes(safe_project_data, safe_analysis_results, company_info, additional_pdf=additional_pdf)
    if main7 is None:
    ...
    try:
        ...
        if additional_pdf:
            # Seitenanzahl ermitteln (gesamt = 7 + n)
            ...
            try:
                ...
                footer_left = None
            additional_pdf = _overlay_footer_page_numbers(additional_pdf, start_number=8, total_pages=total_pages, logo_b64=logo_b64, footer_left_text=footer_left)
...
def _add_product_details_to_story(
    ...
    def _get_attr_value_safe(pid: Optional[Union[int, float]], akey: str) -> Optional[str]:
        if pid in (None, ""):
            return None
        try:
    ...
    for key_prod, label_text_key_prod in all_fields_to_display_prod:
        ...
        # Fallback: Für Modul-spezifische kanonische Felder ggf. aus Attribute-Tabelle lesen
        if (value_prod is None or str(value_prod).strip() == "") and cat_lower_prod == 'modul' and key_prod in (
            ...
            value_prod = _get_attr_value_safe(product_details.get('id'), key_prod)
        ...
        if value_prod is not None and str(value_prod).strip() != "":
            ...
            # Für reine Textfelder (kanonische Modul-Felder) nicht numerisch formatieren
            if key_prod in ('cell_technology','module_structure','cell_type','version','module_warranty_text'):
...
def generate_offer_pdf(
    ...
    try:
        ...
        if not cust.get('last_name'):
            # Fallback: aus project_details (z. B. customer_name) oder Platzhalter
            pd = project_data.get('project_details') or {}
            fullname = pd.get('customer_full_name') or pd.get('customer_name') or ''
            if isinstance(fullname, str) and fullname.strip():
                # Split: letzter Token als Nachname
                parts = fullname.strip().split()
                if parts:
                    cust['last_name'] = parts[-1]
                    cust['first_name'] = ' '.join(parts[:-1]) if len(parts) > 1 else cust.get('first_name', '')
            if not cust.get('last_name'):
                cust['last_name'] = 'Kunde'
        project_data['customer_data'] = cust

        # 4) Fehlende KPIs anlage_kwp / annual_pv_production_kwh defensiv ableiten
        if not analysis_results.get('anlage_kwp'):
            ...
            try:
                ...
                kwp = (mod_qty * mod_wp) / 1000.0 if mod_qty > 0 and mod_wp > 0 else None
            except Exception:
                kwp = None
        ...
        if not analysis_results.get('annual_pv_production_kwh'):
            ...
            if isinstance(kwp, (int, float)) and kwp > 0:
                analysis_results['annual_pv_production_kwh'] = float(kwp) * 1000.0
    except Exception:
