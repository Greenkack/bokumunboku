...
def render_pdf_ui(
    ...
    offer_type = st.radio("Angebotstyp", ["Photovoltaik", "Wärmepumpe", "Kombiniert Photovoltaik + Wärmepumpe"], key="radio_offer_type_selection")
    ...
    if 'pdf_inclusion_options' not in st.session_state:
        st.session_state.pdf_inclusion_options = {
            ...
            "append_additional_pages_after_main7": False,
    ...
    if offer_type == "Photovoltaik":
        ...
        if not hp_offer:
            ...
            if not (project_details_pdf.get('module_quantity') and (project_details_pdf.get('selected_module_id') or project_details_pdf.get('selected_module_name'))):
                ...
                project_details_pdf.setdefault('selected_module_name', 'Placeholder Modul')
                project_details_pdf.setdefault('selected_inverter_name', 'Placeholder WR')
                project_details_pdf.setdefault('anlage_kwp', 0.1)
            project_data['heatpump_offer'] = hp_offer
            pv_total = analysis_results.get('total_investment_netto') if isinstance(analysis_results, dict) else None
            hp_total = hp_offer.get('beg_subsidy', {}).get('effective_total_after_subsidy_net')
            if isinstance(analysis_results, dict):
                analysis_results['combined_final_price_netto'] = (pv_total or 0) + (hp_total or 0)
    ...
    companies_list = st.session_state.get('all_companies_list') or []  # Erwartet Liste von Dicts mit id,name
    if companies_list and isinstance(companies_list, list):
        with st.expander("Optionale separate Firmen für Photovoltaik und Wärmepumpe wählen"):
            pv_company_names = [c.get('name','Firma?') for c in companies_list]
            ...
            if active_company:
                for idx, c in enumerate(companies_list):
                    if c.get('id') == active_company.get('id'):
                        pv_default_index = idx; wp_default_index = idx; break
            pv_sel = st.selectbox("Firma Photovoltaik", pv_company_names, index=pv_default_index, key="pv_company_select")
            wp_sel = st.selectbox("Firma Wärmepumpe", wp_company_names, index=wp_default_index, key="wp_company_select")
            # IDs speichern
            try:
                pv_company_id = next((c.get('id') for c in companies_list if c.get('name') == pv_sel), None)
                wp_company_id = next((c.get('id') for c in companies_list if c.get('name') == wp_sel), None)
        ...
        st.caption("(Keine Firmenliste für getrennte Photovoltaik/Wärmepumpe Auswahl im Session-State gefunden)")
    ...
    if offer_type == "Kombiniert Photovoltaik + Wärmepumpe" and st.session_state.get('heatpump_offer'):
        ...
        if pv_ts and hp_ts:
            segment_order = ['Photovoltaik','Wärmepumpe'] if pv_ts <= hp_ts else ['Wärmepumpe','Photovoltaik']
    ...
    with hp_cols1[0]:
        st.session_state.pdf_inclusion_options['include_hp_line_items'] = st.checkbox("Einzelpreise (Netto/Förderung/Nach Förderung)", value=st.session_state.pdf_inclusion_options.get('include_hp_line_items', True), key='hp_inc_line_items')
        st.session_state.pdf_inclusion_options['include_hp_subsidy_block'] = st.checkbox("Förderquote & absolut", value=st.session_state.pdf_inclusion_options.get('include_hp_subsidy_block', True), key='hp_inc_subsidy_block')
    with hp_cols1[1]:
        st.session_state.pdf_inclusion_options['include_hp_total_price_block'] = st.checkbox("Gesamtpreis Netto/MwSt/Brutto", value=st.session_state.pdf_inclusion_options.get('include_hp_total_price_block', True), key='hp_inc_total_price')
        st.session_state.pdf_inclusion_options['include_hp_financing_block'] = st.checkbox("Finanzierung (Monatsrate)", value=st.session_state.pdf_inclusion_options.get('include_hp_financing_block', True), key='hp_inc_financing')
    with hp_cols1[2]:
        st.session_state.pdf_inclusion_options['include_hp_benefits_bullets'] = st.checkbox("Nutzen-Bullets", value=st.session_state.pdf_inclusion_options.get('include_hp_benefits_bullets', True), key='hp_inc_benefits')
        st.session_state.pdf_inclusion_options['include_hp_custom_texts'] = st.checkbox("Individuelle Textbausteine", value=st.session_state.pdf_inclusion_options.get('include_hp_custom_texts', True), key='hp_inc_custom_texts')
    ...
    with st.form(key="pdf_generation_form_v13_final_locked_options_main", clear_on_submit=False):
        ...
        # Schalter: Zusatzseiten ab Seite 8
        col_pdf_content1_form, col_pdf_content2_form, col_pdf_content3_form = st.columns(3)
        with col_pdf_content1_form: 
            # NEU: Klarer Hauptschalter für erweiterte PDF-Ausgabe (steuert Zusatzseiten ab Seite 8)
            st.session_state.pdf_inclusion_options["append_additional_pages_after_main7"] = st.checkbox(
                get_text_pdf_ui(texts, "pdf_append_after_main7_label", "Erweiterte PDF-Ausgabe (ab Seite 8) aktivieren?"),
                value=st.session_state.pdf_inclusion_options.get("append_additional_pages_after_main7", False),
                key="pdf_cb_append_after_main7_v1"
            )
            append_after_main7_flag = bool(st.session_state.pdf_inclusion_options.get("append_additional_pages_after_main7", False))

            # Zusatzoptionen für Seiten ab 8 nur anzeigen, wenn der Schalter aktiv ist
            if append_after_main7_flag:
                st.caption("Erweiterter Modus aktiv: Die 7 festen Hauptseiten werden erzeugt und zusätzliche Angebotsseiten ab Seite 8 angehängt.")
                ...
                st.info(get_text_pdf_ui(texts, "pdf_only_main7_active_info", "Standardmodus: Es werden nur die 7 Hauptseiten erzeugt. Erweiterte PDF-Ausgabe ist deaktiviert."))
